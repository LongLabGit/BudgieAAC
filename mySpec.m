function [power, timeLabel, origPower] = mySpec(signal, timeWin, sampleRate, ifplot, bgThresh)
%timeWin in Sec
...
[S,F,T,P]=spectrogram(audSig,256,256-0.001*sampleRate, 300:5:7e3,sampleRate);  %8ms window, stepSize 1ms
origPower = P;
logP = 10*log10(P);
%logP = log10(P);
logP(isinf(logP)) = nan;
...
if ifplot
    imagesc(1:size(power, 2), 300:5:7e3, power);
    set(gca,'ydir','normal');
    colormap('turbo');
