function [T, S] = vigiSpec(varargin)
%input (in that order!)
%   w: signal, 1 x time
%   fs: sample freq (hz)
...
%   freeze colors: this is an advanced option if you want to plot something on top of the spectrogram without affecting the spectrogram itself
%   reference point for origin 0.
defaultArgs= {[], [], 500:5:8e3,.7,512,384,[],0, 0};%this is standard for bird
optargs=defaultArgs;
optargs(1:nargin)=varargin(1:nargin);
optargs(cellfun(@isempty,optargs))=defaultArgs(cellfun(@isempty,optargs));
[w,fs,F,thr,win,step,tR,rgb,Toffset] = optargs{:};
